...
@export var fog_emission: Color = Color(0.04, 0.10, 0.22, 0.08)
...
@export var beam_emission: Color = Color(0.35, 0.75, 1.3, 1.0)
@export var beam_energy: float = 18.0
@export var beam_alpha: float = 0.32

@export var rung_count: int = 6
@export var rung_energy: float = 10.0
...
@export var ring_emission: Color = Color(0.50, 0.95, 1.50, 1.0)
@export var ring_energy: float = 11.0
@export var ring_height_span: float = 1.0

@export var top_light_energy: float = 5.0
@export var bottom_light_energy: float = 4.0

# Ground/floor panel (large, bottom)
@export var floor_radius: float = 220.0         # spans the whole view
@export var floor_thickness: float = 0.15
@export var floor_y_offset: float = -60.0       # push far below the tower
@export var floor_color: Color = Color(0.05, 0.08, 0.12, 1.0)
@export var floor_emission: Color = Color(0.18, 0.40, 0.85, 0.55)
@export var floor_emission_energy: float = 1.6
...
func _add_beam_and_lights() -> void:
	...
	m.albedo_color = Color(0.15, 0.35, 0.55, 1.0)
	...
	for i in range(rung_count):
		var t: float = float(i) / float(max(rung_count - 1, 1))
		var y: float = lerp(-fog_size.y * 0.45, fog_size.y * 0.65, t)
	...
	for i in range(ring_count):
		var t_ring: float = float(i) / float(max(ring_count - 1, 1))
		var y_ring: float = lerp(-span * 0.5, span * 0.5, t_ring)
...
func _add_floor_panel() -> void:
	...
	floor.position = Vector3(0, floor_y_offset, 0)
	add_child(floor)
