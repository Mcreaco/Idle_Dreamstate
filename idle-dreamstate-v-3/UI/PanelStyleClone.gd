...
@export var reference_dreamcloud_path: NodePath
...
func _ready() -> void:
	var ref: dreamcloud = null

	# 1) Prefer explicit path
	if reference_dreamcloud_path != NodePath("") and reference_dreamcloud_path != null:
		ref = get_node_or_null(reference_dreamcloud_path) as Control
	...
	if ref == null:
		push_warning("PanelStyleClone: couldn't find reference. Set reference_dreamcloud_path or reference_node_name.")
		return
